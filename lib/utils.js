 ...
 */
const addLocationChangeCallback = (callback, { runAtInit = true } = {}) => {
  if (runAtInit) {
 ...
 * @returns {string}
 */
const parseImgSrcset = (srcset = '') => {
  let bestSrcset = ''
  if (srcset) {
    const parsedSrcset = parseSrcset(srcset)
    if (parsedSrcset && parsedSrcset.length) {
      const highestSrc = parsedSrcset.reduce(({ width, density }, acc) =>
  ...
  if (data && data.length) {
    for (const imageEl of data) {
      ...
      if (!id) {
        let bestSrc = src
        if (srcset) {
          const bestSrcset = parseImgSrcset(srcset)
          if (bestSrcset) {
            bestSrc = bestSrcset
          }
        }
